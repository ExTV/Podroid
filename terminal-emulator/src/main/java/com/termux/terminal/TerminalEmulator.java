...
public final class TerminalEmulator {
    ...
    /**
     * DECSET 2026 — Synchronized Output (a.k.a. "atomic frame" / "sync update mode").
     * https://gitlab.com/gnachman/iterm2/-/wikis/synchronized-updates-spec
     *
     * When the application brackets a frame in `\e[?2026h ... \e[?2026l`, the
     * terminal must defer rendering until the close bracket arrives, so the
     * user never sees a partially-updated screen. Used by btop, nvim, fzf,
     * lazygit — eliminates flicker on TUIs that redraw frequently.
     */
    ...
    /**
     * Kitty graphics protocol — multi-chunk base64 PNG accumulator.
     * https://sw.kovidgoyal.net/kitty/graphics-protocol/
     *
     * Minimum viable: `\e_Ga=T,f=100,t=d,m=0/1; <base64 PNG chunk> \e\`
     * Used by: kitty +kitten icat, lazygit thumbnails, lf preview, fzf preview,
     * yazi, ranger, hyperfine plot output.
     *
     * The first chunk carries all parameters; continuation chunks (m=1/m=0)
     * carry only `m=`, so action/format/transport must persist.
     */
    private StringBuilder mKittyB64Accumulator;
    private char mKittyAction;
    private int mKittyFormat;
    private char mKittyTransport;
    /** Display columns (`c=`) and rows (`r=`); 0 = natural size scaled to a sensible cap. */
    private int mKittyCols;
    private int mKittyRows;
    ...
    /**
     * True while the application is between `\e[?2026h` and `\e[?2026l`.
     * Callers (TerminalSession) skip view invalidation while this is true so
     * the user never sees a half-redrawn frame.
     */
    ...
    private void doDcs(final int b) {
        if (
            ...
            // Podroid: also flush on `$` (sixel CR) and `-` (sixel LF). The upstream sixel4
            // fork only triggers on `#`/`!`/`"`, but in pixel-data runs those are infrequent
            // (color is typically set once per row) while `$` and `-` end every sub-row. With
            // chafa's high-color output the buffer would otherwise overflow at 16 KB before
            // any flush-eligible byte arrived — see Podroid CLAUDE.md.
            || (ESC_DCS__SIXEL && ((b == '#' || b == '!' || b == '"' || b == '$' || b == '-') &&
                    ((mTerminalControlArgs.length() >= (TERMINAL_CONTROL_ARGS__DEFAULT_MAX_LENGTH / 2)) || (mTerminalControlArgs.length() > 1 && mSixelCommandPartNum == 1))))
        ) {
                ...
                if (ESC_DCS__CHECK_IF_SIXEL && !ESC_DCS__SIXEL) {
                    ...
                    char ch = mTerminalControlArgs.charAt(0);
                    if (ch == 'q' || (ch >= '0' && ch <= '9')) {
                        startIfSixelDcs();
                    } else {
    ...
    private int[] getSixelDcsSetupArgs(String dcs, int index) {
        ...
        while (index < dcs.length()) {
            ...
            if (ch >= '0' && ch <= '9') {
                ...
                // Podroid: also accept `Pa;Pb;Pcq` form (no trailing `;` before `q`).
                // chafa emits `\eP0;1;0q` for Sixel; the upstream sixel4 fork only handled
                // the `Pa;Pb;Pc;q` form (with trailing `;`), so chafa output never entered
                // Sixel mode and bytes accumulated as DCS args until overflow at 16 KB.
                return args;
            } else {
                break;
            }
            ...
            // - https://vt100.net/docs/vt3xx-gp/chapter14.html#S14.3.2
                ...
                if (isValidDcs && arg == 3) { // If complete spec is received and is valid.
                    ...
                    if (sixelWidth > 0 && sixelHeight > 0) {
                        ...
                        int sixelArgsPartsExpectedLength = (int) ((((double) TERMINAL_CONTROL_ARGS__DEFAULT_MAX_LENGTH / 2) + sixelWidth) * 1.02);
    ...
    private void doCsiBiggerThan(int b) {
        switch (b) {
            case 'c': // "${CSI}>c" or "${CSI}>c". Secondary Device Attributes (DA2).
                ...
                break;
            case 'q':
                // XTVERSION (`CSI > 0 q`) — report terminal name + version.
                // Format: `DCS > | NAME ST` per xterm spec.
                // Modern apps (Neovim ≥0.10, kitty-aware tools) use this to detect
                // capabilities (truecolor, sync output, etc.) without trial-and-error.
                if (getArg0(0) == 0) {
                    String version = mClient != null ? mClient.getTerminalVersionString() : null;
                    if (version == null || version.isEmpty()) version = "Podroid";
                    mSession.write("\033P>|" + version + "\033\\");
                }
                break;
            case 'm':
    ...
    private void receiveApc(final int b) {
        switch (b) {
            case 27: // Escape.
                ...
                break;
            default:
                // Kitty graphics chunks can be 4–64 KB each; raise the cap as soon as we
                // detect `\e_G…`. 1 MB ceiling keeps a malicious or runaway payload from
                // exhausting memory.
                if (mTerminalControlArgs.length() == 0 && b == 'G') {
                    setTerminalControlArgsMaxLength(1024 * 1024);
                }
                if (!collectTerminalControlArgs(b)) return;
        }
    }
    ...
    /**
     * APC dispatcher. Currently only Kitty graphics (`G…`) is recognized.
     * Format: `\e_G<key>=<value>,<key>=<value>,...; <base64-payload> \e\`
     * Multi-chunk: `m=1` more coming, `m=0` (or absent) last chunk.
     */
    private void doApc() {
        String args = mTerminalControlArgs.toString();
        if (args.isEmpty() || args.charAt(0) != 'G') return;

        int sep = args.indexOf(';');
        String header = sep >= 0 ? args.substring(1, sep) : args.substring(1);
        String payload = sep >= 0 ? args.substring(sep + 1) : "";

        // First chunk seeds defaults; continuation chunks (m=1/m=0) only carry `m=`,
        // so any key not present preserves its prior value.
        boolean firstChunk = (mKittyB64Accumulator == null);
        if (firstChunk) {
            mKittyAction = 'T';     // spec default
            mKittyFormat = 32;      // spec default (RGBA) — we only handle 100 (PNG)
            mKittyTransport = 'd';  // spec default
            mKittyCols = 0;
            mKittyRows = 0;
        }
        ...
        for (String pair : header.split(",")) {
            ...
            switch (key) {
                case "a": if (!val.isEmpty()) mKittyAction = val.charAt(0); break;
                case "f": try { mKittyFormat = Integer.parseInt(val); } catch (Exception e) {} break;
                case "t": if (!val.isEmpty()) mKittyTransport = val.charAt(0); break;
                case "c": try { mKittyCols = Integer.parseInt(val); } catch (Exception e) {} break;
                case "r": try { mKittyRows = Integer.parseInt(val); } catch (Exception e) {} break;
                case "m": moreChunks = "1".equals(val); break;
                default: break;
            }
        }

        // Only "transmit and display" with PNG via base64-direct is supported.
        if (mKittyAction != 'T' || mKittyFormat != 100 || mKittyTransport != 'd') {
            mKittyB64Accumulator = null;
            return;
        }

        if (mKittyB64Accumulator == null) mKittyB64Accumulator = new StringBuilder();
        mKittyB64Accumulator.append(payload);

        if (moreChunks) return; // wait for more

        // Final chunk: decode + place via the existing iTerm/byte-array path.
        try {
            byte[] png = android.util.Base64.decode(mKittyB64Accumulator.toString(),
                android.util.Base64.DEFAULT);
            // Convert cell-count display dims to pixel dims if specified.
            int displayW = mKittyCols > 0 ? mKittyCols * mCellWidthPixels : 0;
            int displayH = mKittyRows > 0 ? mKittyRows * mCellHeightPixels : 0;
            int[] cursorDelta = mScreen.addTerminalBitmapForImage(
                png, mCursorCol, mCursorRow,
                mCellWidthPixels, mCellHeightPixels,
                displayW, displayH, /* preserveAspectRatio */ true);
            // cursorDelta[0]=column delta, cursorDelta[1]=row delta — emit linefeeds for vertical advance.
            for (int i = 0; i < cursorDelta[1]; i++) doLinefeed();
            if (cursorDelta[0] != 0) {
                mCursorCol = Math.min(mCursorCol + cursorDelta[0], mColumns - 1);
            }
        } catch (Throwable t) {
            Logger.logError(mClient, LOG_TAG, "Kitty graphics decode failed: " + t.getMessage());
        } finally {
            mKittyB64Accumulator = null;
        }
    }
    ...
    private void doOsc(String bellOrStringTerminator) {
        ...
        switch (value) {
            ...
            case 12: // Set cursor color.
                ...
                break;
            case 8: {
                // OSC 8 hyperlink — `\e]8;<params>;<url>\e\` opens, `\e]8;;\e\` closes.
                // textParameter holds `<params>;<url>` (or empty `;` to close).
                int sep = textParameter.indexOf(';');
                String url = sep >= 0 ? textParameter.substring(sep + 1) : "";
                if (url.isEmpty()) {
                    mScreen.endHyperlink(mCursorRow, mCursorCol);
                } else {
                    mScreen.beginHyperlink(url, mCursorRow, mCursorCol);
                }
                break;
            }
